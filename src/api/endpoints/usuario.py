...
# POST Usuario / Signup
@router.post(
    "/signup", status_code=status.HTTP_201_CREATED, response_model=UsuarioSchema
)
...
async def get_usuarios(
    ...
    async with db as session:
        ...
        if usuario_logado.permissao_id == 3:
            raise HTTPException(
                detail="O usuário logado não tem permissão para ver essas informações",
...
async def put_usuario(
    ...
    async with db as session:
        ...
        if usuario_logado.id != usuario_id and usuario_logado.permissao_id != 1:
            raise HTTPException(
                detail="O usuário logado não tem permissão para modificar este usuário.",
                status_code=status.HTTP_401_UNAUTHORIZED,
            )
        ...
        if usuario_up:
            ...
            if usuario.permissao_id:
                if usuario_logado.permissao_id != 1:
                    raise HTTPException(
                        detail="O usuário logado não tem permissão para modificar a permissão deste usuário.",
                        status_code=status.HTTP_401_UNAUTHORIZED,
                    )
...
async def delete_usuario(
    ...
    async with db as session:
        ...
        if usuario_logado.id != usuario_id and usuario_logado.permissao_id != 1:
            raise HTTPException(
                detail="O usuário logado não tem permissão para deletar este usuário.",
                status_code=status.HTTP_401_UNAUTHORIZED,
            )
