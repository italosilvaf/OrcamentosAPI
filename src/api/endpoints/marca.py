...
async def post_marca(marca: MarcaSchemaBase, usuario_logado: UsuarioModel = Depends(get_current_user), db: AsyncSession= Depends(get_session)):
    nova_marca: MarcaModel = MarcaModel(nome=marca.nome)

    if usuario_logado.permissao_id != 1:
        raise HTTPException(detail='O usuário logado não tem permissão para criar marcas.',
...
async def put_marca(marca_id: int, marca: MarcaSchemaUp, usuario_logado: UsuarioModel = Depends(get_current_user), 
    ...
    async with db as session:
        ...
        if usuario_logado.permissao_id != 1:
            raise HTTPException(detail='O usuário logado não tem permissão para editar marcas.',
                                    status_code=status.HTTP_401_UNAUTHORIZED)
...
async def delete_produto(marca_id: int, usuario_logado: UsuarioModel = Depends(get_current_user), db: AsyncSession = Depends(get_session)):
    async with db as session:
        query = select(MarcaModel).filter(MarcaModel.id == marca_id)
        result = await session.execute(query)
        ...
        if usuario_logado.permissao_id != 1:
            raise HTTPException(detail='O usuário logado não tem permissão para deletar marcas.',
                                    status_code=status.HTTP_401_UNAUTHORIZED)
