...
# POST Produto
@router.post('/', response_model=ProdutoSchemaBase, status_code=status.HTTP_201_CREATED)
async def post_produto(produto: ProdutoSchemaBase, db: AsyncSession= Depends(get_session)):
...
async def put_produto(produto_id: int, produto: ProdutoSchemaUp, db: AsyncSession = Depends(get_session)):
    async with db as session:
        query = select(ProdutoModel).filter(ProdutoModel.id == produto_id)
        result = await session.execute(query)
